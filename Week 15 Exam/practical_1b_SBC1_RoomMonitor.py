...
DEST_S267_IPV6 = '2001:DB8:ACAD:1::3'
PORT = 5000
...
def send_instruction_S256(instruction):
    ...
    delay(50)
    client.close()  
def send_instruction_S267(instruction):
    """ Sends an instruction message to a TCP client at a specified IPv6 address and port. """
    client = TCPClient()
    client.connect(DEST_S267_IPV6, PORT)
    delay(50)
    client.send(instruction)
    delay(50)
    client.close()    
...
def main():
    ...
    while True:
        ...
        if total_free_pc != EMPTY:
            send_instruction_S256(total_free_pc)
            print("S265: {}".format(total_free_pc))
            del room_list[:]
            print("S265: {}".format(total_free_pc))
        sleep(4)
        http.get(API_URL.format('s267'))
        if total_free_pc != EMPTY:
            send_instruction_S267(total_free_pc)
            print("S267: {}".format(total_free_pc))
            del room_list[:]
            print("S267: {}".format(total_free_pc))
        sleep(4)
